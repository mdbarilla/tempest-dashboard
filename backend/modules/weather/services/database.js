...
class WeatherDatabase {
  ...
  initializeDatabase() {
    this.db.serialize(() => {
      ...
      // Add wind_lull column for wind range display (lull/avg/gust)
      this.db.run(`ALTER TABLE observations ADD COLUMN wind_lull REAL`, (err) => {
        if (err && !/duplicate column name/i.test(err.message)) console.error('observations wind_lull:', err);
      });
      ...
      // Guestbook — one row per signature, tagged by visit_id
      this.db.run(`
      ...
      this.db.run(`
        CREATE INDEX IF NOT EXISTS idx_guestbook_visit
        ON guestbook(visit_id, signed_at DESC)
      `);
  ...
  saveObservation(data) {
    return new Promise((resolve, reject) => {
      ...
      const windLull = data.wind?.lull != null ? data.wind.lull : null;
      const sql = `
        INSERT OR REPLACE INTO observations (
          timestamp, temp_celsius, temp_fahrenheit,
          feels_like_celsius, feels_like_fahrenheit,
          humidity, wind_speed, wind_gust, wind_lull, wind_direction,
